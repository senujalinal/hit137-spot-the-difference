...
# both canvases share the same fixed size so the layout stays symmetrical
CANVAS_W = 510
CANVAS_H = 510
# red for correctly found differences, blue for revealed ones, orange for wrong clicks
FOUND_COLOUR = "#FF3B3B"
...
class ImageCanvas(tk.Canvas):
    ...
    def __init__(self, parent, label_text, **kwargs):
        ...
        # scale and offsets are calculated when an image loads and reused for circle drawing
        self._scale = 1.0
        self._offset_x = 0
        self._offset_y = 0
    ...
    def set_image(self, bgr_image):
        ...
        self.delete("all")
        # OpenCV loads BGR but PIL and tkinter expect RGB
        rgb = cv2.cvtColor(bgr_image, cv2.COLOR_BGR2RGB)
        img_h, img_w = rgb.shape[:2]

        # fit-inside scaling keeps the full image visible without distortion
        self._scale = min(CANVAS_W / img_w, CANVAS_H / img_h)
        disp_w = int(img_w * self._scale)
        disp_h = int(img_h * self._scale)
        # centre the image on the canvas with equal padding on each side
    ...
    def draw_circle(self, region, colour, tag="", label=""):
        ...
        x, y, w, h = region
        # convert from image coordinates to canvas coordinates using the stored scale
        cx = self._offset_x + int((x + w / 2) * self._scale)
        cy = self._offset_y + int((y + h / 2) * self._scale)
        # radius grows with the region size but never goes below 28px so it stays clickable
    ...
    def flash_cross(self, canvas_x, canvas_y):
        # Flash a cross at wrong click position, auto-removes after 600 ms.
        s = 14
        tag = "wrong_flash"
        ...
        self.create_line(canvas_x + s, canvas_y - s,
                         canvas_x - s, canvas_y + s,
                         fill=WRONG_COLOUR, width=3, tags=tag)
        # use after() so the cross disappears automatically without blocking the UI
        self.after(600, lambda: self.delete(tag))

    def canvas_to_image_coords(self, canvas_x, canvas_y):
        # Convert canvas pixel coords to original image coords.
        # reverse the scale and offset applied when the image was displayed
        ix = int((canvas_x - self._offset_x) / self._scale)
        iy = int((canvas_y - self._offset_y) / self._scale)
        return ix, iy

    def _draw_placeholder(self):
        # shown before any image is loaded so the canvas doesn't look empty
...
class GameWindow:
    ...
    def _configure_window(self):
        ...
        # centre the window on whatever screen the player is using
        x = (sw - self.MIN_W) // 2
        y = (sh - self.MIN_H) // 2
    ...
    def _build_header(self):
        ...
        # score sits in the header so it's always visible no matter where the player looks
    ...
    def _build_toolbar(self):
        ...
        # reveal starts disabled so the player can't skip the challenge immediately
    ...
    def _build_canvases(self):
        ...
        # only the modified canvas listens for clicks; the original is for comparison only
    ...
    def _build_status_bar(self):
        ...
        # pinned to the bottom so there's always a place for feedback messages
    ...
    def _on_load_image(self):
        ...
        # dialog returns empty string if the player cancels without picking a file
        if not path:
            return
        if not self._processor.load_image(path):
            messagebox.showerror("Error", "Could not open image.")
            return
        # wipe mistakes, found set, and lockout so the new image starts clean
        ...
        # enable reveal now that there's actually an image to reveal
    ...
    def _on_canvas_click(self, event):
        ...
        if self._processor.original_bgr is None:
            return
        # ignore clicks after the round has ended to avoid confusing state changes
        ...
        if result["hit"]:
            ...
            # number the circles in the order the player finds them
    ...
    def _on_reveal(self):
        ...
        # disable the button so it can't be pressed twice on the same image
    ...
    def _on_locked_out(self):
        # reveal remaining differences so the player can see what they missed
        self._state.reveal_all()
        self._draw_revealed_circles()
        self._reveal_btn.config(state="disabled")
        found = self._state.found_count
        ...
        messagebox.showwarning("Too Many Mistakes ",
                               ...
                               f"You found {found} of 5 differences.\n\n"
    ...
    def _update_hud(self):
        ...
        # once the round is over the remaining count should always read zero
        if self._state.is_round_over:
            self._remaining_var.set("Remaining: 0")
        else:
        ...
        # colour the mistakes counter green -> yellow -> red as it gets worse
        if m == 0:
            self._mistakes_lbl.config(fg=TEXT_MUTED)
        elif m < GameState.MAX_MISTAKES:
            self._mistakes_lbl.config(fg=WARNING)
        else:
            self._mistakes_lbl.config(fg=DANGER)

    def _draw_revealed_circles(self):
        found = self._state.found_indices
        for idx, alt in enumerate(self._processor.alterations):
            # only draw blue circles on the ones the player didn't already find
            if idx not in found:
